...
### 現在の本番経路（PC不要・外部cron起動）

```text
外部cron（cron-job.org）
  平日 各枠の時刻（slot: 1135/1535/1705/2005/2355）
    → GitHub API workflow_dispatch
      → daily_update.yml
        → TDnet取得・解析
        → Google Drive保存（rclone）
        → tdnet-viewerへpush
        → GitHub Pages反映
```

GitHub Actions側の`schedule`は同じワークフローを予備起動できますが、遅延しうるため**本番の時刻源にはしません**。
...
ローカル経路はXBRL解析、公開JSON作成、`tdnet-viewer`更新を行いません。  
現行本番（外部cron → Actions）が正常なら`TDnet_Daily_Auto_Local`は無効のままで問題ありません。
...
- cron-job.org: **本番の時刻起動**（`workflow_dispatch`）
- TDnet: 一覧HTML、PDF、XBRL ZIP
- Yahoo Finance / yfinance: PBR、予想PER、配当利回り、株価騰落率、時価総額
- GitHub Actions: 日次処理の実行環境
- GitHub Pages: 静的Web公開
- Google Drive（rclone）: PDF・CSVの長期保存
